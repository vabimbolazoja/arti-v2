...
import {
    ChevronLeft, MapPin, Phone, CreditCard, MoreVertical, Flag, Ban, Mic, Camera, X,
    CheckCircle2, AlertCircle, Search, Download, Share2, Copy, Plus
} from 'lucide-react';
import { MESSAGES } from '../../constants/userData';
...
const MessagesView = ({ messagesViewStep, setMessagesViewStep, currentChat, setCurrentChat, chatMessages, setChatMessages, searchMessages, setSearchMessages, zoomedImage, setZoomedImage, showMenuModal, setShowMenuModal, selectedReportOption, setSelectedReportOption, setCurrentView }) => {
    ...
    useEffect(() => {
        let interval;
        if (isRecording) { interval = setInterval(() => { setRecordingTime(prev => (prev < 9 ? prev + 1 : 0)); }, 1000); }
    ...
    const renderReport = () => (
        <div className="flex-1 lg:ml-[280px] bg-white min-h-screen flex flex-col pt-24 lg:pt-0">
            ...
            <div className="mt-auto p-6 pb-12">
                <button disabled={!selectedReportOption} onClick={() => { if (selectedReportOption === 'Other') setMessagesViewStep('report_other'); else setMessagesViewStep('feedback'); }} className={`w-full py-5 rounded-2xl font-black text-lg transition-all shadow-xl active:scale-95 ${selectedReportOption ? 'bg-[#1E4E82] text-white' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Submit</button>
            </div>
        </div>
    );
    ...
    const renderReportOther = () => (
        <div className="flex-1 lg:ml-[280px] bg-white min-h-screen flex flex-col pt-24 lg:pt-0">
            ...
            <div className="mt-auto p-6 pb-12"><button onClick={() => setMessagesViewStep('feedback')} className="w-full py-5 bg-[#1E4E82] text-white rounded-2xl font-black text-lg shadow-xl active:scale-95 transition-all">Submit</button></div>
        </div>
    );
            ...
            <div className="flex-1 p-6 space-y-8 overflow-y-auto pb-32 scroll-smooth">
                ...
                {chatMessages.map((msg, idx) => (
                    <div key={idx} className={`flex flex-col ${msg.sender === 'user' ? 'items-end ml-auto' : 'items-start'} gap-1 max-w-[85%]`}>
                        {msg.type === 'text' && <div className={`${msg.sender === 'user' ? 'bg-[#1E4E82] text-white rounded-tr-none' : 'bg-[#F1F5F9] text-[#0f172a] rounded-tl-none'} p-4 font-medium text-base rounded-[24px] leading-relaxed shadow-sm`}>{msg.content}</div>}
                        {msg.type === 'image' && <div onClick={() => setZoomedImage(msg.content)} className="rounded-[24px] overflow-hidden border-4 border-white shadow-xl cursor-pointer hover:scale-[1.02] transition-transform max-w-[240px]"><img src={msg.content} alt="Sent attachment" className="w-full h-full object-cover max-h-64" /></div>}
                        {msg.type === 'invoice' && (
                        ...
                        <div className="flex items-center gap-1.5 px-1 mt-1"><span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">{msg.time}</span>{msg.sender === 'user' && <CheckCircle2 size={12} className="text-blue-400" />}</div>
                    </div>
                ))}
            </div>
            <div className="fixed bottom-0 lg:sticky lg:bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-gray-100 px-6 py-4 pb-10 lg:pb-6 z-40">
                <div className="max-w-4xl mx-auto flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={() => { const newMsg = { id: Date.now(), type: 'image', content: 'https://images.unsplash.com/photo-1595113316349-9fa4ee24f884?auto=format&fit=crop&q=80&w=600', sender: 'user', time: '2:30pm' }; setChatMessages(prev => [...prev, newMsg]); }} className="p-3 text-gray-400 hover:text-blue-900 bg-gray-50 rounded-full transition-colors active:scale-90"><Camera size={26} /></button>
                        <div className="flex-1 relative">
                            <input type="text" placeholder="Type a message..." className="w-full pl-6 pr-14 py-3.5 rounded-full border border-gray-100 bg-white focus:outline-none focus:ring-2 focus:ring-blue-100 shadow-inner font-medium text-sm"
                                onKeyDown={(e) => { if (e.key === 'Enter' && e.target.value.trim()) { const newMsg = { id: Date.now(), type: 'text', content: e.target.value, sender: 'user', time: '2:30pm' }; setChatMessages(prev => [...prev, newMsg]); e.target.value = ''; } }} />
                            <button className={`absolute right-2 top-1/2 -translate-y-1/2 p-2.5 rounded-full transition-all ${isRecording ? 'text-red-500 bg-red-50 scale-125' : 'text-[#1E4E82] hover:bg-blue-50'}`} onClick={() => { if (!isRecording) { setIsRecording(true); setRecordingTime(0); } else { setIsRecording(false); } }}><Mic size={24} /></button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    const renderInvoiceDetail = () => (
        <div className="flex-1 lg:ml-[280px] bg-white min-h-screen flex flex-col pt-24 lg:pt-0">
            ...
            <div className="fixed lg:sticky bottom-0 left-0 right-0 p-6 bg-white border-t border-gray-100 space-y-4">
                ...
                <button onClick={() => { setChatMessages(prev => [...prev.filter(m => m.type !== 'invoice'), { id: Date.now(), type: 'rejected', sender: 'artisan', time: '2:30pm' }]); setMessagesViewStep('chat'); }} className="w-full py-5 text-[#1E4E82] font-black text-base active:scale-95 transition-all uppercase tracking-widest">Reject Invoice</button>
            </div>
        </div>
    );
